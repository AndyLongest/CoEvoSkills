...
# Bibliography Verification Skill

Detect fake or hallucinated citations in a BibTeX file.

## Execution

**Execute this script.** It writes `/root/answer.json`.

### Step 1: Install dependencies
```bash
pip install bibtexparser python-dateutil
```

### Step 2: Run
```bash
python /app/environment/skills/evo-bib-verify/scripts/verify_bib.py
```

### Step 3: Verify
```bash
python -c "import json; d=json.load(open('/root/answer.json')); print('OK:', len(d['fake_citations']))"
```

**The script writes `/root/answer.json`. Do not write it manually.**

## Heuristics

Entry flagged fake if:
1. Missing required fields (title, author, year)
2. Title = "undefined", "unknown", "title", "test", placeholder, all digits, no letters
3. Author: no capitals, too short, random string
4. Year < 1800 or > now + 2
5. Volume 0 or > 1000
6. Pages: invalid, start > end, range > 500
7. DOI invalid
