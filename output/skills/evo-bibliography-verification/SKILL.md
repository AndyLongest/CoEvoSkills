---
name: evo-bibliography-verification
---

# Bibliography Verification

Detect fake or hallucinated citations in a BibTeX file. Parse the file, validate each entry against the CrossRef API (DOI lookup or title search), and apply conservative heuristics when the network is unavailable.

Use when you need to:
- Validate citations before manuscript submission
- Check a `.bib` file for fabricated or incomplete references
- Ensure only real, verifiable papers are cited

## Workflow

1. **Parse the BibTeX file** – Read with `bibtexparser` (or regex fallback for truncated entries). Extract all titles.
2. **Validate each entry** – For each citation with a title:
   - If DOI: query `https://api.crossref.org/works/{doi}`. HTTP 200 = real. HTTP 404 = fake.
   - If no DOI: search CrossRef with `query.title`. Found = real. Not found + heuristic match = fake.
3. **Conservative flagging** – Only flag with strong evidence:
   - DOI returns 404
   - Entry missing both `author` AND `year` (incomplete/truncated)
   - DOI uses test prefix (`10.1234`, `10.5678`, `10.0000`) AND generic author name
4. **Validate output** – Every title in `fake_citations` must exist in the BibTeX file. No hallucinated titles.
5. **Clean and output** – Remove braces, backslashes. Sort alphabetically. Write to `/root/answer.json`.

## Functions

### `detect_fake_citations(bib_path, output_path)`

- **Inputs**: `bib_path` (str), `output_path` (str)
- **Purpose**: Full validation with API + heuristic fallback. Writes JSON.
- **Returns**: `list` of fake titles (sorted, deduplicated, validated)

### `heuristic_only(bib_path)`

- **Inputs**: `bib_path` (str)
- **Purpose**: Offline heuristic detection.
- **Returns**: `list` of fake titles

## Usage

```python
import sys
sys.path.insert(0, '/app/environment/skills/evo-bibliography-verification/scripts')
from verify_bib import detect_fake_citations

titles = detect_fake_citations('/root/test.bib', '/root/answer.json')
print(titles)
```
...
```python
from verify_bib import heuristic_only
import json
titles = heuristic_only('/root/test.bib')
with open('/root/answer.json', 'w') as f:
    json.dump({"fake_citations": titles}, f, indent=2)
```

---

```python filename=scripts/verify_bib.py
"""
